...
allocate(posmpi(ndim,ppc,fitnum))               !position of particles per core per objective
allocate(fitvalloc(fitnum,ppc,fitnum))          !fitness values of particles per core per objective
posmpi(:,:,:)=0.0
fitvalloc(:,:,:)=1000
                ...
                do i=1,swarmsize,1
                        do j=1,ndim,1
                                do k=1,fitnum,1
                                        ...
                                        !write(*,*) randn
                                        pos(j,i,k)=limits(j,1,k)+(randn * limits(j,3,k))
                                enddo
                        end do
                        !write(*,'(22 F8.4)') pos(:,i)
                enddo
...
do i=1,maxiter,1
call MPI_SCATTER(pos, ppc*ndim*fitnum, MPI_DOUBLE_PRECISION, posmpi, ppc*ndim*fitnum, MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, ierror)
call MPI_SCATTER(fitval, ppc*fitnum*fitnum, MPI_DOUBLE_PRECISION, fitvalloc, ppc*fitnum*fitnum, MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, ierror)
call MPI_BARRIER(MPI_COMM_WORLD, ierror)
...
call MPI_GATHER(fitvalloc, ppc*fitnum*fitnum, MPI_DOUBLE_PRECISION, fitval, ppc*fitnum*fitnum, MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, ierror)
